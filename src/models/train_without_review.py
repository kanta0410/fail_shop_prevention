"""
Nagoya Restaurant Closure Prediction - Training Pipeline (Without Review Features)

レビューやレーティングに関連する特徴量の予測寄与度が高すぎる（リークや過学習の懸念がある）ため、
それらに関連する全ての特徴量を完全に除外した状態でのベースラインモデルを学習し、SHAP分析を行うスクリプト。
出力はすべて data/output/without_review ディレクトリに保存される。
"""
...
OUTPUT_DIR = 'data/output/without_review'
os.makedirs(OUTPUT_DIR, exist_ok=True)
RANDOM_STATE = 42

# ---------------------------------------------------------
# 【レビュー・レーティング関連を除外した特徴量リスト】
# ---------------------------------------------------------

# 連続値系（標準化対象）
CONTINUOUS_COLS = [
    # ローデータ由来のレーティング・レビュー特徴量（rating, log_review_count等）は除外
    # エリア相対値のレーティング・レビュー特徴量（rating_diff_500m等）も除外
    'price_diff_500m',
...
FLAG_COLS = [
    ...
    'price_unknown_flag', 
    # 'high_rating_low_review_flag' は除外
...
def apply_clustering(X_train, X_val, X_test, df):
    ...
    # Val / Test は DBSCAN の性質上直接 predict できないため、単純化してKNNや一律ノイズ扱い、
    # もしくは今回は単純にis_dense_area=0としてフォールバックする。
...
def plot_shap(model, X, model_name):
    ...
    plt.title(f'SHAP Feature Importance ({model_name} - Without Review)')
    plt.tight_layout()
    ...
    plt.title(f'SHAP Beeswarm Plot ({model_name} - Without Review)')
    plt.tight_layout()
...
def main():
    print("=" * 60)
    print("Starting Training Pipeline WITHOUT REVIEW FEATURES")
    print("=" * 60)
    ...
    print("\n============================================================")
    print("Final evaluation on TEST set (Without Review)...")
    test_results = []
    ...
    pd.DataFrame(test_results).to_csv(os.path.join(OUTPUT_DIR, 'test_metrics_without_review.csv'), index=False)
    
    print("\n============================================================")
    print("All steps completed successfully in without_review module!")
