...
def main():
    ...
    print(f"Loaded feature dataset shape: {df_features.shape}")
    ...
    # 既にマージ済みのカラムがある場合は一度ドロップする (衝突防止)
    for col in ['floor', 'primary_type', 'floor_num', 'floor_is_other']:
    ...
    # フロア特徴量はテナントマッチング精度の問題により一旦全て削除
    pass

    # 5. 特徴量化 (店舗カテゴリ - Label Encoding)
    # LightGBM向けにOne-Hotではなく単一の整数コードに変換
    ...
    # 多重共線性（価格ダミー）の罠を避けるため、price_unknown をドロップ
    ...
    print(f"\n[SUCCESS] New final 2026 dataset saved to {output_path} (Shape: {df_final.shape})")

    # 統計出力
    closed_counts = df_final['is_closed'].value_counts()
    print("\n=== Statistics for the New 2026 Dataset ===")
    ...
    print(f"  New added columns: primary_type_code")
